// Menu toggle is handled by React onClick in EnglishPage.jsx — no legacy setup needed.
...
// Fetch certificate data from external JSON file
async function loadCertificates() {
    try {
        const response = await fetch('/assets/__data/certificateDetails.json'); // Adjust the path if needed
        if (!response.ok) {
...
async function openServiceModal(eventId) {
    ...
    timeElement.textContent = `Processing Time: ${event.time}`;

    currentImages = event.images;
    currentSlide = 0;

    if (currentImages.length > 0) {
        carousel.innerHTML = `<img src="${currentImages[0]}" class="w-full h-full object-cover rounded-lg" alt="Event Image">`;
    } else {
        console.error("No images found in currentImages.");
    }

    modal.classList.remove("hidden");
    document.body.style.overflow = "hidden";
    document.body.style.translate = 'none';
